...
export default async function AttractionPage({ params }: { params: Params }) {
  ...
  // Only promise free cancellation when at least one option on this page actually offers it.
  const anyFreeCancel = tours.some((t) => t.cancellationPolicy !== 'non-refundable');
  const cancelNote = anyFreeCancel ? 'free cancellation on most' : 'non-refundable tickets';
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 pb-24">
        ...
        <div className="mt-4 max-w-3xl">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm text-gray-500">{tours.length} option{tours.length === 1 ? '' : 's'} &middot; {cancelNote}</span>
          </div>
        </div>
        ...
        <div className="mt-12 rounded-xl bg-gradient-to-r from-green-700 to-emerald-800 p-5 sm:p-6 text-center text-white">
          ...
          <p className="text-sm text-green-100 mb-4">Compare all {a.name} options and book instantly through GetYourGuide. {anyFreeCancel ? 'Free cancellation on most.' : 'Tickets are non-refundable, so check your date first.'}</p>
          {top && (
