...
import { getMonthBySlug, monthStrip } from '@/data/nyc-months';
import { nycClimate, CLIMATE_CONTEXT } from '@/data/nyc-climate';
...
/** Snowfall reads better as "none" than as "0cm" in the months where the normal is zero. */
function snowLabel(cm: number): string {
  if (cm === 0) return 'None, the normal is zero';
  if (cm < 1) return `${cm}cm, a trace`;
  return `${cm}cm`;
}
...
export function buildMonthPage(slug: string) {
  ...
  const climate = nycClimate[data.monthKey];
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    headline: `New York in ${data.month}: Weather, Events and the Best Tours`,
  ...
  // BreadcrumbList is emitted by the shared Breadcrumbs component below, so it is
  // not repeated here.
  const faqPageSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
  ...
  const itemListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `The best tours in New York in ${data.month}`,
    ...
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        <Breadcrumbs
          items={[
            { label: 'Home', href: '/' },
            { label: 'Best time to visit New York', href: HUB_PATH },
            { label: `New York in ${data.month}` },
          ]}
        />
        ...
        <article>
          <header>
            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 leading-tight">
              New York in {data.month}: Weather, Events and the Best Tours
            </h1>
          ...
          <section aria-labelledby="quick-pick" className="mt-8">
            <h2 id="quick-pick" className="text-xl font-bold text-gray-900">
              Quick pick: the best New York tours in {data.month}
            </h2>
            <div className="mt-3 overflow-x-auto rounded-lg border border-gray-200">
              <table className="w-full min-w-[640px] text-left text-sm">
                <caption className="sr-only">
                  Our ten ranked New York tours for {data.month}, with what each is best for
                </caption>
          ...
          {/* 1. Verdict */}
          <H2 id="is-it-good">Is {data.month} a good time to visit New York?</H2>
          ...
          {/* 2. Weather */}
          <H2 id="weather">New York weather in {data.month}</H2>
          <Capsule>{data.weather.capsule}</Capsule>
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[560px] text-left text-sm">
              <caption className="sr-only">New York climate and daylight averages for {data.month}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                <tr>
                  <th scope="row" className="px-3 py-2.5 font-medium text-gray-900">Precipitation</th>
                  <td className="px-3 py-2.5 text-gray-800">{climate.precipMm}mm</td>
                  <td className="px-3 py-2.5 text-gray-600">{CLIMATE_CONTEXT.precip}</td>
                </tr>
                <tr>
                  <th scope="row" className="px-3 py-2.5 font-medium text-gray-900">Snowfall</th>
                  <td className="px-3 py-2.5 text-gray-800">{snowLabel(climate.snowCm)}</td>
                  <td className="px-3 py-2.5 text-gray-600">{CLIMATE_CONTEXT.snow}</td>
                </tr>
                <tr>
          ...
          <p className="mt-2 text-xs text-gray-500">
            ...
            , converted from Fahrenheit and inches. Daylight, sunrise and sunset are for the 15th, from{' '}
          ...
          {/* 4. Tours */}
          <H2 id="best-tours">The best tours in New York in {data.month}</H2>
          ...
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[640px] text-left text-sm">
              <caption className="sr-only">Dated events in New York in {data.month} {year}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
          ...
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[640px] text-left text-sm">
              <caption className="sr-only">Closures and reduced schedules affecting New York sights in {data.month}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
                  <th scope="col" className="px-3 py-2.5 font-semibold">Site or service</th>
                  <th scope="col" className="px-3 py-2.5 font-semibold">Closed or reduced</th>
          ...
          <p className="mt-2 text-xs text-gray-500">
            Harbour cruise season bands are published by the operator on{' '}
            <a href={CRUISE_SOURCE.url} target="_blank" rel="noopener noreferrer nofollow" className="underline">
              {CRUISE_SOURCE.label}
            </a>
            . Statue of Liberty and Ellis Island opening days come from{' '}
            <a href={LIBERTY_SOURCE.url} target="_blank" rel="noopener noreferrer nofollow" className="underline">
              {LIBERTY_SOURCE.label}
            </a>
            . Check both before you book a dated slot.
          </p>
          ...
          {/* 7. Packing */}
          <H2 id="what-to-pack">What to wear and pack for New York in {data.month}</H2>
          ...
          <ol className="mt-4 space-y-3 text-gray-700 leading-relaxed list-decimal pl-5">
            ...
            <li>
              ...
              for a specific, stated reason tied to {data.month}: temperature, snowfall, sunset time, an
              operator season band or a closure. Every move is explained in the entry itself rather than left
              implied.
            </li>
            <li>
              <span className="font-semibold text-gray-900">Closed means out.</span> If a product is not
              running, or an operator has cut its sailings for the season, it does not sit in the ten
              unflagged however popular it is. We name it in the closures table instead so you can plan
              around it.
            </li>
            <li>
              <span className="font-semibold text-gray-900">Nothing is invented.</span> Prices, star ratings
              and review counts come from the live product page, checked on {TOUR_DATA_CHECKED}. Where an
              official 2027 date had not been published when we wrote this, we describe the usual pattern and
              say plainly that the date is not confirmed.
            </li>
            <li>
          ...
          {/* 9. FAQs */}
          <H2 id="faqs">FAQs about New York in {data.month}</H2>
          ...
          <nav aria-label="Other months" className="mt-12">
            <div className="flex flex-col sm:flex-row gap-3 justify-between">
              ...
              >
                ...
                <span className="block font-semibold text-blue-900">New York in {data.prevMonth}</span>
              </Link>
              <Link
                href={data.nextPath}
              ...
              >
                ...
                <span className="block font-semibold text-blue-900">New York in {data.nextMonth}</span>
              </Link>
            </div>

            <div className="mt-6 rounded-lg bg-gray-50 border border-gray-200 p-4">
              <p className="text-sm font-semibold text-gray-900">New York month by month</p>
...
export function buildMonthMetadata(slug: string) {
  ...
  if (!data) return {};
  const year = seasonYear(data.yearOffset);
  const title = `New York in ${data.month} ${year}: Weather, Events and the Best Tours`;
  return {
